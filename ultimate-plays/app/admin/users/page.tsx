...
export default function AdminUsersPage() {
  ...
  const [newRole, setNewRole] = useState<"member" | "editor" | "admin">("member");
  ...
  async function handleRoleChange(email: string, role: "member" | "editor" | "admin") {
    try {
      const res = await fetch("/api/admin/users", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          ...
          <form onSubmit={handleAdd} className="flex gap-3 flex-wrap items-end">
            ...
            <div>
              ...
              <select
                value={newRole}
                onChange={(e) => setNewRole(e.target.value as "member" | "editor" | "admin")}
              ...
              {users.map((u) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className={`shrink-0 inline-block text-xs font-medium px-2 py-0.5 rounded-full ${roleBadgeClass(u.role)}`}>
                      {u.role}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <select
                      value={u.role}
                      onChange={(e) => handleRoleChange(u.email, e.target.value as "member" | "editor" | "admin")}
                      className="rounded border border-gray-200 px-2 py-1 text-xs text-gray-700 focus:outline-none focus:ring-1 focus:ring-green-600"
                    >
                      <option value="member">Member</option>
                      <option value="editor">Editor</option>
                      <option value="admin">Admin</option>
